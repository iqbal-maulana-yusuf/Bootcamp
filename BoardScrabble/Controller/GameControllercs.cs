...
namespace BoardScrabble.Controller
{
    public class GameControl
    {
        ...
        public MoveError PerformTurn(IPlayer player, List<TilePlacement> placements)
        {
            ...
            System.Diagnostics.Debug.WriteLine(" jumlah kata yang terdeteksi " + formedWords[0]);
            foreach (var word in formedWords)
            {
                if (!IsWordInDictionary(word))
                {
                    System.Diagnostics.Debug.WriteLine("Checkpoint 8");
            ...
            System.Diagnostics.Debug.WriteLine("Checkpoint 11");

            // 7. Isi ulang rak pemain
            RefillPlayerRack(player);

            System.Diagnostics.Debug.WriteLine("Checkpoint 12");

            _consecutiveSkips = 0; // Reset skip count on successful move
            OnGameEvent?.Invoke("TurnCompleted", player);
            System.Diagnostics.Debug.WriteLine("Checkpoint 13");

            NextTurn();
            CheckEndGameConditions();

            System.Diagnostics.Debug.WriteLine("Checkpoint 14");
